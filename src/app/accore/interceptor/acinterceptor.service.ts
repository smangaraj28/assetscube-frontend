...
import { environment, installation } from '../../../environments/environment';
...
export class AcinterceptorService {
  ...
  constructor(/*private ses_serv: session_service*/
   ...
   intercept(req: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    ...
    this.headers = this.setHeader(this.headers, 'site', installation.siteid.toString());
    const screenid = this.genericservice.get_screen_id();
    console.log("print site");
    console.log(installation.siteid.toString());
...
check_url_present(urlarray: string[],req: HttpRequest<any>): boolean {
  let isurlpresent:boolean = false;
  urlarray.forEach( (ues) => {
    let r = req.url.endsWith(ues);
    if(!isurlpresent) {
      isurlpresent = r;
    }
  });
