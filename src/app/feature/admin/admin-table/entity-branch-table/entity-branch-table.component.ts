...
export class EntityBranchTableComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    fetchAllRowAPICall() {
        this.dataService.getAllIssues().subscribe(successResponse => {
                this.onFetchSuccess(successResponse);
            },
            errorResponse => {
    ...
    onFetchSuccess(successResponse) {
        console.log(successResponse);
        this.refreshTableData(successResponse);
    ...
    refreshTableData(resolvedEntityBranchData) {
        if (('t' in resolvedEntityBranchData[0]) &&
            (resolvedEntityBranchData[0]['t'] === 'data couldn\'t be extracted from fetch results')) {
            this.entityBranchDataSource = [];
            this.clonedEntityBranchDataSource = [];
        } else {
            this.entityBranchDataSource = [...resolvedEntityBranchData];
    ...
    onSave() {
        ...
        switch (this.subCardLabel) {
            case 'Add':
                // Think of IDEA RXJS operator For After Save API Call -> Fetch API Call Should happen
                this.dataService.addIssue(this.entityBranchData);
